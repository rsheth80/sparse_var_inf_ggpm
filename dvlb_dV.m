function [dvlbdV,dvlbdVV] = dvlb_dV(model,data,params,calcs)
% function [dvlbdV,dvlbdVV] = dvlb_dV(model,data,params,calcs)
%
...
dvlbdV_lik = C18*bsxfun(@times,C18',0.5*expdf2);

% vlb gradient due to KL divergence:
invV = params.var.V\eye(size(params.var.V));
if(~iscalcs)
    dvlbdV_kld = 0.5*(invV - L'\(L\eye(size(L))));
else
    dvlbdV_kld = 0.5*(invV - calcs.iKmm);
end;

% total gradient
dvlbdV = dvlbdV_kld + dvlbdV_lik;
